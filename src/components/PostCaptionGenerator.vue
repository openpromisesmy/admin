<template>
  <collapsible-card title="PROMISE MADE Caption">
    ...
    <div id="caption-text">
      ...
      <p v-if="promise.description">
        <b>DESCRIPTION:</b>
        {{ promise.description }}
      </p>
      <p v-if="promise.elaboration">
        <b>ELABORATION:</b>
        {{ promise.elaboration }}
      </p>
      <p v-if="promise.context">
        <b>CONTEXT:</b>
        {{ promise.context }}
      </p>
      <p v-if="promise.clauses.broken">
        <b>BROKEN CLAUSE:</b>
        {{ promise.clauses.broken }}
      </p>
...
export default {
  ...
  methods: {
    ...
    copyCaption () {
      ...
      this.$toast.success(
        `Caption copied to your clipboad! - ${this.promise.title}`
      )
    }
  },
  computed: {
    captionText: function () {
      ...
      return {
        project_info:
          'OpenPromises Malaysia is a Malaysian homegrown non-partisan project. None of our members are affiliated with any of the political parties and our intention of running this project is to increase the quality and integrity of our representatives.',
